...
if st.session_state.logged_in:
    ...
    if not df.empty:
        ...
        df_user = df[df["Assegnato a"].str.lower() == username.lower()]

        if df_user.empty:
            st.warning("Nessun task trovato per l'utente loggato!")
        else:
            # -----------------------------
            # Metriche principali
            # -----------------------------
            st.metric("Totale Task", len(df_user))
            st.metric("Task Completati", len(df_user[df_user["Stato"].str.lower() == "completato"]))
            st.metric("Story Points Totali", df_user["Story Points"].sum())
            ...
            # Tasks per settimana
            df_user["Settimana"] = df_user["Data fine"].dt.to_period("W").astype(str)
            tasks_week = df_user.groupby("Settimana").size()
            ...
            # Tasks per Stato
            tasks_state = df_user["Stato"].value_counts()
            ...
            # Velocity
            velocity = df_user.groupby("Assegnato a")["Story Points"].sum()
            ...
            # Kanban semplice
            # -----------------------------
            kanban.show_kanban(df_user)

            # -----------------------------
            # Esportazione report
            # -----------------------------
            if st.button("📥 Esporta PDF"):
                st.download_button("Scarica PDF", reports.export_pdf(df_user), "report.pdf")
            if st.button("📥 Esporta Excel"):
                st.download_button("Scarica Excel", reports.export_excel(df_user), "report.xlsx")

            # -----------------------------
            # Notifiche task in ritardo
            # -----------------------------
            overdue = notifications.check_overdue_tasks(df_user)
